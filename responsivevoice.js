...
$jscomp.scope = {};
    ...
    function k(m, l) {
        ...
        a.fallbackBufferLength = 150;
        a.iOS && (a.fallbackBufferLength = 150);
        var q = null,
            z = 1;
                    ...
                    if (g) {
                        ...
                        if (b.length > a.CHARACTER_LIMIT) {
                            for (var p = b = f(b); p.length > a.CHARACTER_LIMIT;) {
                                ...
                                if(u.length>1&& !u.startsWith('.'))
                                {
                                    a.countTextProcees +=u.length;
        ...
        // play audio
        //a.fallbackChunkIndex : vị trí phân tử trong mảng hiện tại ;
        //fallbackChunks : danh sách tất cả các object có url và text (audio)
        ...
        a.initFallbackPool =
            function() {
                if (!a.fallback_audiopool || 0 == a.fallback_audiopool.length) {
                    for (var b = 0; a.fallbackBufferLength > b; b++) {
                        var c = b,
    ...
    let panelBodyChild =panelBody.querySelector('.row.margin-bottom-lg')

    let divElement = document.createElement('div')
    if(responsiveVoice.is_android )
    {
        divElement.innerHTML = '<div class="left" style="display: flex; flex-grow: 1;align-items: center"><button class="play-button"><i class="fa fa-play"></i></button><button class="mute-button activeVolume"><i class="fa fa-volume-up" aria-hidden="true"></i></button><div class="volume-slider" id="volume-slider"><div class="volume-slider-handle"></div></div></span><span class="current">0:00</span>/<span class="duration">0:00</span><div class="progress-bar" id="progress-bar"><span class="bg"></span><div class="wrap-recent"><span class="fill-recent" ><span class="current_time"></span></span></div></div></div><div class="right"><i class="fa fa-microphone" aria-hidden="true"></i><select id="voiceselection" class="input input--dropdown js--animations"><i class="fa fa-microphone" aria-hidden="true"></i><option class="voice-female" id="option-voice" value="Vietnamese Female">Giọng nữ Việt Nam</option><option class="voice-male" id="option-voice" value="Vietnamese Male"><i class="fa fa-microphone" aria-hidden="true"></i>Giọng nam Việt Nam</option></select></div>';

        console.log('thiết bị android ');
    }
    else if (responsiveVoice.iOS){
        divElement.innerHTML = '<div class="left" style="display: flex; flex-grow: 1;align-items: center"><button class="play-button"><i class="fa fa-play"></i></button><button class="mute-button activeVolume"><i class="fa fa-volume-up" aria-hidden="true"></i></button><div class="volume-slider" id="volume-slider"><div class="volume-slider-handle"></div></div></span><span class="current">0:00</span>/<span class="duration">0:00</span><div class="progress-bar" id="progress-bar"><span class="bg"></span><div class="wrap-recent"><span class="fill-recent" ><span class="current_time"></span></span></div></div></div><div class="right"><i class="fa fa-microphone" aria-hidden="true"></i><select id="voiceselection" class="input input--dropdown js--animations"><i class="fa fa-microphone" aria-hidden="true"></i><option class="voice-female" id="option-voice" value="Vietnamese Female">Giọng nữ Việt Nam</option><option class="voice-male" id="option-voice" value="Vietnamese Male"><i class="fa fa-microphone" aria-hidden="true"></i>Giọng nam Việt Nam</option></select></div>';
    ...
    {
        divElement.innerHTML = '<div class="left" style="display: flex; flex-grow: 1;align-items: center"><button class="play-button"><i class="fa fa-play"></i></button><button class="mute-button activeVolume"><i class="fa fa-volume-up" aria-hidden="true"></i></button><div class="volume-slider" id="volume-slider"><div class="volume-slider-handle"></div></div></span><span class="current">0:00</span>/<span class="duration">0:00</span><div class="progress-bar" id="progress-bar"><span class="bg"></span><div class="wrap-recent"><span class="fill-recent"><span class="current_time"></span></span></div></div></div><div class="right"><i class="fa fa-microphone" aria-hidden="true"></i><select id="voiceselection" class="input input--dropdown js--animations"><i class="fa fa-microphone" aria-hidden="true"></i><option class="voice-female" id="option-voice" value="Vietnamese Female">Giọng nữ Việt Nam</option><option class="voice-male" id="option-voice" value="Vietnamese Male"><i class="fa fa-microphone" aria-hidden="true"></i>Giọng nam Việt Nam</option></select></div>';
        console.log('thiết bị latop,tablet');
    ...
    panelBodyChild.appendChild(divElement);
    let btnPlay = document.querySelector('.play-button');
    let btnMute = document.querySelector('.mute-button');
    // let btnMute = document.querySelector('.left');
    ...
    Object.assign(btnPlay.querySelector('i').style,{
       display:'flex',
    ...
    !responsiveVoice.is_android&&!responsiveVoice.iOS?Object.assign(document.querySelector('.progress-bar').style,{
        backgroundColor:'#e9ecef',
        position: 'relative',
        marginLeft: '16px',
        marginRight: '16px',
    ...
    }):Object.assign(document.querySelector('.progress-bar').style,{
        backgroundColor:'#e9ecef',
        marginLeft: '16px',
        marginRight: '16px',
        height: '100%',
    ...
    !responsiveVoice.is_android&&!responsiveVoice.iOS?Object.assign(document.querySelector('.bg').style,{
        position: 'absolute',
        display: 'flex',
        height: '0',
        top: '29px',
    ...
    }):Object.assign(document.querySelector('.bg').style,{
        position: 'absolute',
        display: 'flex',
        height: '0',
        top: '0',
    ...
    !responsiveVoice.is_android&&!responsiveVoice.iOS?Object.assign(document.querySelector('.fill-recent').style,{
        position: 'absolute',
        display: 'flex',
        height: '0',
        top: '29px',
    ...
    Object.assign(document.querySelector('.fill-recent').style,{
        position: 'absolute',
        display: 'flex',
        height: '0',
        top: '0',
    ...
    Object.assign(document.querySelector('.current_time').style,{
        position: 'absolute',
        right: '-7px',
        top: '-5px',
        width: '7px',
    ...
    Object.assign(document.querySelector('.right').querySelector('#voiceselection').style,{
        width:'159px',
        height:'34px',
        borderRadius: '3px',
        backgroundColor: 'rgba(255,255,255,.9)',
    ...
    Object.assign(document.querySelector('.right').querySelector('#option-voice').style,{
        width:'159px',
        height:'25px',
    ...
    // handle click event btnPlay and pause
    var timePresent =0;
    var textCout ;
    let durationTime = 0;
    let seconds;
    ...
    for(var i = 1; i < panelBody.children.length;i++)
    {
        contextBody+=panelBody.children[i].innerText;
    }
    ...
    const handleCurrentTime = () => {
        ...
        if(currentSeconds>=seconds&&currentMinutes>=minutes)
        {
            currentSeconds=seconds;
            currentMinutes=minutes;
        }
        // if (responsiveVoice.endedVoice) {
            
        // }
        //progress-bar
        let totalTimeCurrent = currentMinutes*60+currentSeconds;
        timePresent = (totalTimeCurrent/durationTime *100).toFixed(2);
        if(timePresent>100)
        {
            timePresent=100;
            if(isFinite(timePresent))
            {
                timePresent =0;
            }
        }
        ...
        if (responsiveVoice.endedVoice) {
            ...
            responsiveVoice.endedVoice=false;
            btnPlay.innerHTML= '<i class="fa fa-play" styles="display:flex,justify-content:center,align-items: center" aria-hidden="true"></i>';
         

        }
        document.querySelector('.current').innerText=`${currentMinutes}:${currentSeconds}`;    
        document.querySelector('.fill-recent').style.width = `${timePresent}%`;
        ...
        // else if(responsiveVoice.endedVoice){
        //     responsiveVoice.speak(panelBody.innerText, document.getElementById('voiceselection').value);
        //     responsiveVoice.endedVoice=false;
        // }
        else{
            btnPlay.innerHTML= '<i class="fa fa-pause" styles="display:flex,justify-content:center,align-items: center" aria-hidden="true"></i>';
            responsiveVoice.setTextReplacements([
                ...
                {
                    searchvalue: "Giọng nam Việt Nam",
                    newvalue: ""
                },
                
                {
                    searchvalue: "Giọng nữ Việt Nam",
                    newvalue: ""
                },
                
            ]);
            //handle voice play and resume
            // check tiemPresent (đang bị pause)
            if(currentMinutes>0 || currentSeconds>0)
            {
                responsiveVoice.resume();
                intercurrent =setInterval(handleCurrentTime, 1000);

            }
            else{
                
                    responsiveVoice.speak(contextBody, document.getElementById('voiceselection').value);
                    //sau 1 s set time 1 lan
                    intercurrent =setInterval(handleCurrentTime, 1000);
                    //sau 8s set time 
                    setTimeout(()=>{
                        document.querySelectorAll('.audio-body').forEach((item)=>{
                           if(!isNaN( item.duration))
                           {
                               durationTime=durationTime + item.duration;
                           }
                                  
                    })
                    minutes = Math.floor(durationTime / 60);
                     seconds = Math.floor(durationTime - minutes * 60);
                   document.querySelector('.duration').innerText = `${minutes}:${seconds}`;
                   },responsiveVoice.fallbackChunks.length * 0.0666*1000);
                
            }
             textCout = responsiveVoice.countTextProcees;
    ...
    //btnPlay
    btnPlay.addEventListener('click',handlePlayVoice)

    //btnVolume
    btnMute.addEventListener('click',() => {
       
        if(btnMute.matches('.activeVolume'))
        {
            btnMute.classList.remove('activeVolume');
            btnMute.innerHTML = '<i class="fa fa-volume-off" aria-hidden="true"/>';
            responsiveVoice.setVolume(0);
        }
        else {
            btnMute.classList.add('activeVolume');
            btnMute.innerHTML = '<i class="fa fa-volume-up" aria-hidden="true"/>';
...
document.getElementById('voiceselection').addEventListener('change', function(){
    //fallbackChunkIndex: ví trí mảng đang chạy hiện tại
    if(responsiveVoice.fallbackChunks)
    {
        responsiveVoice.pause();
        responsiveVoice.fallbackChunks.forEach((item,index)=>{
                try {
                    item.audio.src&&item.audio.src.includes('gender=female')?
                    item.audio.src =item.audio.src.replace('gender=female','gender=male'):item.audio.src =item.audio.src.replace('gender=male','gender=female');
                } catch (error) {
                    responsiveVoice.speak(contextBody, document.getElementById('voiceselection').value);
                } 
        })
